...
// Get all events a user has registered for (excluding cancelled)
export const findByUser = async (user_id) => {
    const res = await db.query(
        `SELECT er.*, e.title, e.description, e.starts_at, e.ends_at, e.location
         FROM event_registrations er
         JOIN events e ON er.event_id = e.id
         WHERE er.user_id=$1 AND er.status='registered'`,
        [user_id]
    );
    return res.rows;
};